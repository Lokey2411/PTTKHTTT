...
const statusChecked = document.querySelectorAll("table td select");
const statusLength = statusChecked.length;
const statusRestored = JSON.parse(localStorage.getItem(selectedValue))
  ? JSON.parse(localStorage.getItem(selectedValue))
  : [];
for (let i = 0; i < statusLength; i++) {
  statusRestored.push(statusChecked[i].value);
}
for (let i = 0; i < statusLength; i++) {
  statusChecked[i].addEventListener("change", function () {
    statusRestored[i] = statusChecked[i].value;
    localStorage.setItem("status", JSON.stringify(statusRestored));
  });
}
const restore = () => {
  for (let i = 0; i < statusLength; i++) {
    const statusStoraged = JSON.parse(localStorage.getItem(selectedValue))[i];
    if (statusStoraged) {
      statusChecked[i].value = statusStoraged;
    }
  }
};
